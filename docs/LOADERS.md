...
1. **DLL 载荷要求服务端有与目标架构一致的 mingw-w64 gcc**（x64 DLL 需要
   `x86_64-w64-mingw32-gcc`；MSYS2: `pacman -S mingw-w64-x86_64-gcc`）。生成载荷页的
   「DLL 载荷」一栏会直接显示本机是否可用、缺哪一个 —— v1.3.6 起 `dll` 格式编译的是
   **真正的 DLL**（`IMAGE_FILE_DLL` + 导出表）；v1.3.5 及以前它其实是"改了扩展名的 EXE"，
   白加黑/rundll32 都加载不了。
2. 把植入端按 **dll** 格式构建（命令里的 `<DLL载荷ID>` 换成实际 build id）。
