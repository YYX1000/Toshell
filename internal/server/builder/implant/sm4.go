...
// 与服务端 internal/common/crypto/sm4.go 完全一致（S-box / FK / CK / 轮函数 / GHASH / GCM）。
// 每帧使用随机 12 字节 nonce + 16 字节认证标签，机密性与完整性兼具。
...
// 调用方需保证 plaintext 底层数组有至少 16 字节尾部余量。
func sm4GCMSeal(plaintext, key, nonce []byte) ([]byte, bool) {
	...
	return append(plaintext, tag[:]...), true
}
...
// sm4RandomNonce 填充 12 字节随机 nonce（供 tunnel.go 在帧缓冲内就地生成）。
func sm4RandomNonce(dst []byte) {
	if len(dst) >= sm4NonceSize {
		_, _ = rand.Read(dst[:sm4NonceSize])
	}
}

// deriveSM4Key 从主加密密钥派生 16 字节 SM4 隧道子密钥（SHA-256 域分离，与服务端一致）。
...
func sm4EncryptTunnel(plaintext, key []byte) []byte {
	...
	sealed, ok := sm4GCMSeal(buf, key, nonce)
	if !ok {
		return plaintext
	}
...
func sm4DecryptTunnel(frame, key []byte) []byte {
	...
	pt, ok := sm4GCMOpen(frame[sm4NonceSize:], key, frame[:sm4NonceSize])
	if !ok {
		return nil
	}
	return pt
