...
而且 `--dev` 用 `go run` 起后端，**不需要先构建** —— 改 Go 代码重启即生效，改植入端模板
连 `sync` 都不用（它直接读模板源码目录）。

> 两种方式是**两个独立实例**：运行时目录不同（`deploy` 在 `release/`，`dev` 在仓库根），
> 因此**配置、SQLite 库、载荷目录都不共享**，而 API 端口默认都是 18081 —— **不能同时跑**。
> 升级到 `--dev` 前请留意：它是一套全新的空实例，监听器需要重新配置。
